...
- [x] Write prototype-focused automated tests before claiming the phase works:
   ...
   - Notes: Created `tests/gateway/test_zulip.py` with ~70 tests across 15 test classes: Platform enum (2), Config loading (8), Chat-ID helpers — stream build/parse/roundtrip/edge-cases (8), Chat-ID helpers — DM build/parse/roundtrip (7), Requirements check (4), Adapter init (4), Format message (6), Connect/Disconnect (4), Self-message filtering (3), Dedup cache (3), Inbound event dispatch — DM/stream/mention/command/edge-cases (9), Outbound send — stream/DM/fallback/errors (6), get_chat_info (4), send_typing (4), edit_message (4), _resolve_typing_target (4), Stream cache (4). All 100 tests pass (test_zulip + test_config). Compileall smoke check clean on gateway, hermes_cli, tests.

- [x] Run the prototype verification loop and fix issues until green:
   ...
   - Notes: All checks green. `test_zulip.py` (86 tests) + `test_config.py` (14 tests) = 100 passed. Full gateway suite: 1514 passed. hermes_cli/test_gateway.py: 18 passed. `compileall gateway hermes_cli tests` clean with no syntax errors. All 7 touched modules import cleanly (`gateway.platforms.zulip`, `gateway.config`, `gateway.run`, `agent.prompt_builder`, `toolsets`, `hermes_cli.gateway`, `hermes_cli.status`, `hermes_cli.config`). No formatter/linter configured in repo (no ruff/black/flake8 in venv or pyproject.toml). No issues to fix.
